...
oauth2.user.download = function(callback) {
	...
	oauth2.get('v1.0/me', {}, function(response) {
...
oauth2.user.reset_password = function(token, password, callback) {
	oauth2.post('v1.0/password/tokens/' + token, {
		"password" : password
	}, function(response) {
		callback()
	}, function(jqXHR, textStatus) {
...
oauth2.user.send_reset_email = function(email, callback) {
	oauth2.post('v1.0/password/tokens', {
		"emailAddress" : email
	}, function(response) {
		callback()
...
oauth2.user.updateName = function(value, callback) {
	...
	oauth2.put('v1.0/users/' + oauth2.cookie.get('userId'), {
